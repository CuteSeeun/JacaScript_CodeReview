...
function Join() {
    ...
    useEffect(() => {
        ...
        // 모든 필드값이 채워져 있는지 확인하여 완료 버튼 활성화/비활성화
        const formFull = Object.values(formData).every(value => value.trim() !== '');
    ...
    const handleEmail = async (e) => {
        ...
        try {
            const response = await axios.post('http://localhost:3333/user/verifyEmail', { email: formData.email });
