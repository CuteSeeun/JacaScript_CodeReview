...
const saveUser = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(201)
      .json({ id: userId, name, userid, passwd, tel, email });
  } catch (error) {
    console.error(error);
...
const loginUser = async (req, res) => {
  ...
  try {
    const [rows] = await pool.query(
      "SELECT uNo FROM user WHERE userid = ? AND passwd = ?",
      [userid, passwd]
    );
    if (rows.length > 0) {
